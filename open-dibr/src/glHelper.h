...
public:
	...
	bool init(InputCamera input, Options options, int out_width, int out_height, float chroma_offset, OutputCamera output) {
		...
		shader.setFloat("out_projection_type", output.projection == Projection::Perspective ? 0.0f : 0.5f);
		if (output.projection == Projection::Equirectangular) {
			shader.setVec2("out_hor_range", output.hor_range);
			shader.setVec2("out_ver_range", output.ver_range);
		}
	...
	void updateOutputParams(OutputCamera outputCamera) {
		...
		if (!outputCamera.isVR) {
			if (outputCamera.projection == Projection::Perspective) {
