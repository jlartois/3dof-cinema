...
uniform vec2 out_hor_range;  // for equirectangular unprojection
uniform vec2 out_ver_range;  // for equirectangular unprojection
...
void main()
{
	...
	else if(out_projection_type < 0.2f){
		if(viewPosition.z < 0){
	...
	// equirectangular projection
		...
		float R = sqrt(viewPosition.x*viewPosition.x + viewPosition.y*viewPosition.y + viewPosition.z*viewPosition.z);
		float theta = asin(viewPosition.y / R);
		float phi = atan(-viewPosition.x, -viewPosition.z);

		float u = (out_hor_range.y - phi  ) / (out_hor_range.y - out_hor_range.x);
		float v = (out_ver_range.y - theta) / (out_ver_range.y - out_ver_range.x);
		float normalised_depth = (R - out_near_far.x) / (out_near_far.y - out_near_far.x);
		gl_Position = vec4(2.0f * u - 1.0f, 1 - 2.0f * v, normalised_depth, 1.0f);
		vertex.uv = vec2(u,v);
	}
